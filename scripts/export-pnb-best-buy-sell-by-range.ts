#!/usr/bin/env node
/**
 * Categorise PNB day-best BUY/SELL into profit-range buckets.
 *
 * Usage:
 *   npx tsx scripts/export-pnb-best-buy-sell-by-range.ts --src reports/pnb-all-best-buy-sell-20260101_20260330.json --out-suffix 20260101_20260330
 *
 * Ranges (inclusive):
 *   High: 1.8% – 3.0%
 *   Mid:  0.9% – 1.7%
...
function parseArgs(argv: string[]): { src: string; outSuffix: string } {
  let src = resolve(REPORTS_DIR, "pnb-all-best-buy-sell-60d.json");
  let outSuffix = "60d";
...
const RANGES: RangeDef[] = [
  { id: "high", label: "3% – 1.8%", minPct: 1.8, maxPct: 100 }, // includes >3% biggest movers
...
function main(): void {
  ...
  const categorizedBuy = new Set(
    buckets.flatMap((b) => b.buyRows.map((r) => r.dateKey)),
  );
  const categorizedSell = new Set(
    buckets.flatMap((b) => b.sellRows.map((r) => r.dateKey)),
  );
  const outsideBuys = buys
    .filter((b) => !categorizedBuy.has(b.dateKey) || !RANGES.some((r) => inRange(b.profitPct, r)))
  ...
  const lines = [
    `# PNB — best BUY & SELL by profit range (last ${src.tradeDaysScanned}d)`,
    "",
    `- **Symbol:** PNB`,
  ...
  const mdPath = resolve(REPORTS_DIR, `pnb-best-buy-sell-by-range-${outSuffix}.md`);
  const jsonPath = resolve(REPORTS_DIR, `pnb-best-buy-sell-by-range-${outSuffix}.json`);
