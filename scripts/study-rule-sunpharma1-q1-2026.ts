#!/usr/bin/env node
/**
 * RuleSUNPHARMA1 backtest report — SUNPHARMA, 1 Jan → 31 Mar 2026, qty 100.
 *
 * Data: Upstox 1m → NSE 15m (warmup from Dec 2025 for SMI).
 *
 * Usage:
 *   npx tsx scripts/study-rule-sunpharma1-q1-2026.ts
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";
import { config } from "../src/config.js";
import { buildIndicatorSnapshots } from "../src/indicators/compute.js";
import { runRuleSunpharma1Backtest } from "../src/backtest/runRuleSunpharma1Backtest.js";
...
async function main(): Promise<void> {
  ...
  const { squareOffPct, smi } = config.ruleSunpharma1;
  console.log(
    `RuleSUNPHARMA1 · SUNPHARMA · ${STUDY_FROM}→${STUDY_TO} · qty ${QTY} · SQ ${squareOffPct}%`,
  );
  ...
  const md: string[] = [
    `# RuleSUNPHARMA1 · SUNPHARMA · Jan–Mar 2026 · qty ${QTY}`,
    "",
    `- **Rule:** RuleSUNPHARMA1 (SMI black↔red cross + ${squareOffPct}% square-off)`,
    `- **Symbol:** SUNPHARMA`,
    `- **Quantity:** **${QTY}**`,
    `- **Window:** ${STUDY_FROM} → ${STUDY_TO}`,
    `- **Session:** ${config.ruleSunpharma1.sessionStart}–${config.ruleSunpharma1.sessionEnd} IST`,
  ...
  const mdPath = resolve(REPORTS, "rule-sunpharma1-q1-2026-qty100.md");
  const jsonPath = resolve(REPORTS, "rule-sunpharma1-q1-2026-qty100.json");
