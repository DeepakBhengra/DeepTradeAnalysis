...
function parseArgs(argv: string[]): { rangeSrc: string; outSuffix: string } {
  let rangeSrc = resolve(REPORTS_DIR, "pnb-best-buy-sell-by-range-60d.json");
  let outSuffix = "60d";
...
async function main(): Promise<void> {
  const { rangeSrc: RANGE_SRC, outSuffix } = parseArgs(process.argv.slice(2));
  ...
  // Use the study window, with extra lookback so RSI/SMI/BB are warm on day 1.
  const toKey = rangePayload.window.to;
  const windowFrom = new Date(`${rangePayload.window.from}T12:00:00+05:30`);
  const warmFrom = new Date(windowFrom.getTime() - 45 * 24 * 60 * 60 * 1000);
  const fromKey = warmFrom.toISOString().slice(0, 10);
  ...
  const base = `pnb-range-indicator-analysis-${outSuffix}`;
