#!/usr/bin/env node
/**
 * Export all day-best BUY/SELL for PNB from a rule-free best-entry study JSON.
 *
 * Usage:
 *   npx tsx scripts/export-pnb-all-best-buy-sell.ts \
 *     --src reports/pnb-best-entry-times-20260101_20260330.json \
 *     --out-suffix 20260101_20260330
 */
...
function parseArgs(argv: string[]): { src: string; outSuffix: string } {
  let src = resolve(REPORTS_DIR, "pnb-best-entry-times-60d.json");
  let outSuffix = "60d";
...
function main(): void {
  ...
  const lines = [
    `# PNB — all best BUY & SELL (${src.window.from} → ${src.window.to}, positive profit only)`,
    "",
    `- **Symbol:** PNB`,
  ...
  const mdPath = resolve(REPORTS_DIR, `pnb-all-best-buy-sell-${outSuffix}.md`);
  const jsonPath = resolve(REPORTS_DIR, `pnb-all-best-buy-sell-${outSuffix}.json`);
