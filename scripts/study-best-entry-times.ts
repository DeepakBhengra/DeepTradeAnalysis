...
async function main(): Promise<void> {
  const { symbol, tradeDays, topN } = parseArgs(process.argv.slice(2));
  ...
  const base = `${dash.tradingSymbol.toLowerCase()}-best-entry-times-${tradeDays}d`;
  ...
  const lines = [
    `# ${dash.tradingSymbol} — best BUY/SELL times (last ${targetDates.length}d, rule-free)`,
    "",
    `- **Symbol:** ${dash.tradingSymbol}`,
