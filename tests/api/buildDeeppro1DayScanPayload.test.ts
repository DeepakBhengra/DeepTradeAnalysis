...
import { buildDeeppro1DayScanPayload } from "../../src/api/buildDeeppro1DayScanPayload.js";
...
describe("buildDeeppro1DayScanPayload", () => {
  beforeEach(() => {
  ...
  it("rejects invalid dates", async () => {
    await expect(
      buildDeeppro1DayScanPayload({ date: "not-a-date" }),
    ).rejects.toThrow(/date/i);
  });

  it("returns a standard day-scan payload shape for the full watchlist", async () => {
    ...
    const payload = await buildDeeppro1DayScanPayload({ date: "2026-05-11" });
