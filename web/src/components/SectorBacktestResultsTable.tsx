...
function hasExit(trade: DeepakDayScanTrade): boolean {
  return trade.exitTimeIst != null && trade.exitPrice != null;
}
...
export function SectorBacktestResultsTable({
  ...
  const exitTrades = trades.filter(hasExit);
  ...
  return (
    <div className="flex flex-col gap-3">
      <section className="border border-kite-border bg-kite-surface p-3">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <p className="m-0 text-[10px] text-kite-muted">
              Run at{" "}
              {new Date(runAt).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })} IST
            </p>
          </div>
        </div>
        ...
        {errors.length > 0 && (
          <ul className="mb-0 list-none space-y-1 p-0 text-xs text-kite-red">
            {errors.map((entry) => (
              ...
              <tbody>
                {trades.map((trade, index) => (
                  ...
                  >
                    ...
                    <td className="px-2 py-1.5 tabular-nums">
                      {formatMetric(trade.profitTarget)}
                    </td>
      ...
      <section className="border border-kite-border bg-kite-surface p-3">
        ...
        {exitTrades.length === 0 ? (
          <p className="m-0 text-xs text-kite-muted">
            No exits yet. Exits appear when profit targets are hit, stop losses trigger, or
            session square-off closes the trade.
          </p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1000px] border-collapse text-xs">
              ...
              <tbody>
                {exitTrades.map((trade, index) => (
                  <tr
                    key={`exit-${trade.tradingSymbol}-${trade.exitTimeIst}-${trade.entryTimeIst}-${index}`}
                    className="border-b border-kite-border/50"
                  >
                    <td className="px-2 py-1.5 font-medium">{trade.tradingSymbol}</td>
                    <td className="px-2 py-1.5">{trade.sector}</td>
