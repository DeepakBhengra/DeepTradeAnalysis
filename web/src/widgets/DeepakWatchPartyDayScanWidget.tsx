...
import { useDeepakWatchPartyDayScan } from "../hooks/useDeepakWatchPartyDayScan";

const DEFAULT_DATE = "2026-05-11";

interface DeepakWatchPartyDayScanWidgetProps {
  isActive: boolean;
  refreshTrigger?: number;
}

export function DeepakWatchPartyDayScanWidget({
  isActive,
  refreshTrigger = 0,
}: DeepakWatchPartyDayScanWidgetProps) {
  const [date, setDate] = useState(DEFAULT_DATE);
  const [watchlistExpanded, setWatchlistExpanded] = useState(false);
  const { data, loading, loadingElapsedSec, error, info, run, stop } = useDeepakWatchPartyDayScan();
    ...
    <div hidden={!isActive}>
      <main className="mx-auto flex max-w-6xl flex-col gap-3 p-3">
        <DayScanRunControls
          ...
          description={
            <>
              Scans {SECTOR_WATCHLIST_SIZE} large-cap stocks for Deepak entries at 10:15 IST with
              Deepak-2 watch-party stop-loss exits. May take 2–15 minutes depending on Kite response
              time.
            </>
          }
        />
        ...
        />
        <DeepakRulesPanel variant="deepak" />
        ...
        {!data && !loading && !error && !info && (
          <section className="border border-kite-border bg-kite-surface p-3 text-xs text-kite-muted">
            Pick a date and click Run Scan to evaluate Watch Party entries (Deepak @ 10:15 with
            Deepak-2 stop-loss) across the sector watchlist.
          </section>
        )}

        {data && <SectorBacktestResultsTable payload={data} showStopSummary />}
      </main>
    </div>
  );
}
