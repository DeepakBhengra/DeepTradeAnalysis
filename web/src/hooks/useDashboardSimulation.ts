...
import { fetchDashboardSimulation } from "../api/client";
...
interface UseDashboardSimulationResult {
...
export function useDashboardSimulation(
  ...
  const fetchAtIndex = useCallback(
    async (index: number): Promise<DashboardPayload | null> => {
      const requestId = ++requestIdRef.current;

      try {
        const payload = await fetchDashboardSimulation(symbol, analysisDate, index);
        ...
        setData(payload);
        setError(null);
        setSessionIndex(payload.simulation?.sessionIndex ?? index);
        sessionIndexRef.current = payload.simulation?.sessionIndex ?? index;

        const count = payload.simulation?.sessionCandleCount ?? 0;
        setSessionCandleCount(count);
        sessionCandleCountRef.current = count;
        setSimulatedTimeIst(payload.simulation?.simulatedTimeIst ?? null);
