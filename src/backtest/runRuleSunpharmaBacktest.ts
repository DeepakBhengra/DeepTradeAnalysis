...
export function collectRuleSunpharmaTradingDates(
  snapshots: IndicatorSnapshot[],
): string[] {
  const { sessionStart, sessionEnd } = config.ruleSunpharma;
...
export function runRuleSunpharmaBacktest(
  snapshots: IndicatorSnapshot[],
  fromDate: string,
  toDate: string,
): DeepakBacktestResult {
  const allDates = collectRuleSunpharmaTradingDates(snapshots);
