...
import {
  deeppro2SignalToTradeSignal,
  evaluateDeeppro2Day,
} from "../rules/deeppro2Decision.js";
import {
  getSectorRank,
  SECTOR_WATCHLIST,
  type SectorName,
...
async function scanSymbol(
  ...
  try {
    ...
    const day = evaluateDeeppro2Day(snapshots, date, {
      ...
      trades: applyStopLossExitsToTrades(
        withOpenTradeMarkPrices(
          day.signals.map((signal) => {
            const tradeSignal = deeppro2SignalToTradeSignal(signal);
            return {
              date,
...
/**
 * Day Scan for Deeppro2 — same SMI cross logic as Deeppro1 on 5-minute candles.
 */
export async function buildDeeppro2DayScanPayload(input: {
  date: string;
