...
/** RuleSUNPHARMA is SUNPHARMA-only — never mixed with the sector watchlist. */
const RULE_SUNPHARMA_ENTRY = {
  tradingSymbol: config.ruleSunpharma.tradingSymbol,
  sector: "Health",
} as const;
...
async function scanSunpharma(
  date: string,
): Promise<{ trades: DeepakDayScanTrade[]; error: DeepakDayScanError | null }> {
  try {
    const dashboardSymbol = resolveDashboardSymbol(RULE_SUNPHARMA_ENTRY.tradingSymbol);
    const candles = await withDayScanSymbolTimeout(
      ...
      RULE_SUNPHARMA_ENTRY.tradingSymbol,
    );
    const snapshots = buildIndicatorSnapshots(candles);
    const day = evaluateRuleSunpharmaDay(snapshots, date);

    return {
      trades: day.signals.map((signal) => {
        const tradeSignal = ruleSunpharmaSignalToTradeSignal(signal);
        return {
          date,
          ...
          sector: RULE_SUNPHARMA_ENTRY.sector,
        };
      }),
      error: null,
    };
    ...
    return {
      trades: [],
      error: {
        tradingSymbol: RULE_SUNPHARMA_ENTRY.tradingSymbol,
        sector: RULE_SUNPHARMA_ENTRY.sector,
...
/**
 * Day Scan for RuleSUNPHARMA — evaluates **SUNPHARMA only**.
 * Does not scan the sector watchlist and does not share Deepak/Deeppro/RulePNB logic.
 */
export async function buildRuleSunpharmaDayScanPayload(input: {
  date: string;
  ...
  const { trades, error } = await scanSunpharma(input.date);
