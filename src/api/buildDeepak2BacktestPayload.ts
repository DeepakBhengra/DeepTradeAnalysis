...
import { runDeepak2Backtest } from "../backtest/runDeepakBacktest.js";
...
export async function buildDeepak2BacktestPayload(input: {
  symbol: string;
  fromDate: string;
  toDate: string;
  ...
  const { trades, summary } = runDeepak2Backtest(
    snapshots,
    input.fromDate,
    input.toDate,
  );
