...
import { runDeepproBacktest } from "../backtest/runDeepproBacktest.js";
...
export async function buildDeepproBacktestPayload(input: {
  symbol: string;
  fromDate: string;
  toDate: string;
  ...
  const { trades, summary } = runDeepproBacktest(
    snapshots,
    input.fromDate,
    input.toDate,
  );
